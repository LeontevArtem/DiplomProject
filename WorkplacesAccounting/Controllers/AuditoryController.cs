...
namespace WorkplacesAccounting.Controllers
{
    public class AuditoryController : Controller
    {
        ...
        public IActionResult Edit(Classes.Auditory auditory)
        {
            try
            {
                System.Data.DataTable Insert = MsSQL.Query($"UPDATE [dbo].[Auditory] SET [Name] = '{auditory.Name}', [CurentResponsibleTeacher] = '{Data.UsersList.Find(x => x.id == auditory.ResponsibleUserId.ToString()).id}' WHERE ID = '{auditory.Id}' ", Data.ConnectionString);
            }
            catch { }
        ...
        {
            System.Data.DataTable Insert = MsSQL.Query($"DELETE FROM [dbo].[Auditory] WHERE ID = '{id}'", Data.ConnectionString);
            return RedirectToAction("Auditories", "Auditory");
        }
    }
}
